 ...
 */
void SX126x::Init( uint16_t Freq )
{
...
//    IoIrqInit( dioIrq );
	...
	// Define the RF frequency Channel
	SetRfFrequency((uint32_t)Freq*1000000);

    if( OPT == 0 )
    {
        SetDio3AsTcxoCtrl( TCXO_CTRL_2_4V, 320 ); //5 ms
 ...
 */
bool SX126x::SetPayload( uint8_t *payload, uint8_t size )
{
    return WriteBuffer( txBaseAddress , payload, size );
}
 ...
 * that if for any reason the Tx is aborted or does not succeed
 *
 * Parameters:
 * 		payload
 * 		size
 *		timeout -> useconds, if timeout = 0,  this feature is disabled
****************************************************************/
bool SX126x::SendPayload( uint8_t *payload, uint8_t size, uint32_t timeout )
{
    ...
    assert(timeout < 131072000);

    SetTx( timeout / 15.625 );

    status.Value = GetStatus().Value;

    assert(status.Fields.ChipMode == SX126X_STATUS_MODE_TX);
 ...
 */
bool SX126x::SetTx( uint32_t timeout )
{
    uint8_t buf[3];
    ...
    return WriteCommand( RADIO_SET_TX, buf, 3 );
}
...
/*!
 * \brief Sets the radio Rx Gain Configuration
 *
 * \param [in]  gain. Rx Boosted is gain: 0x96

 */
void SX126x::SetRxGain( uint8_t gain )
{
    ...
    WriteReg( REG_RX_GAIN, gain ); // max LNA gain, increase current by ~2mA for around ~3dB in sensivity
...
/* DIO2 can be configured to drive an RF switch
 * DIO2 will be at a logical 1 during Tx and at a logical 0 in any other mode
 *
